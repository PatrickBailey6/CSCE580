...
class MCAgent:
    ...
    def solve(self, initial_missionaries, initial_cannibals):
        ...
        def grab_state(s):
            return (s.left_missionaries, s.left_cannibals,
                    s.right_missionaries, s.right_cannibals,
                    s.boat_position)

        def dfs(): # depth-first search (DFS)
            initial_state = States(initial_missionaries, initial_cannibals, 0, 0, "left")
            start = grab_state(initial_state)
            if initial_state.goal_state():
                return initial_state
            
            stack = [(initial_state, iter(successors(initial_state)), {start}, [initial_state])]
            while stack:
                current, next_iterator, path_set, path_list = stack[-1] # Grab from end
                if current.goal_state():
                    return path_list
                try:
                    next = next(next_iterator)
                    next_state = grab_state(next)
                    if next_state in path_set:
                        continue
                    new_path_set = set(path_set)
                    new_path_set.add(next_state)
                    new_path_list = path_list + [next]
                    stack.append((next, iter(successors(next)), new_path_set, new_path_list))
